...
//如果注销则清空localstorage
    ...
    if(jumpfrom !== '0') {
        ...
        if (autologin == 'true') {
            ...
            //将给客户端发ip地址的操作封装成异步在setwinapp之后执行
                var testpromise = new Promise(function(resolve, reject){
                    var sendDataurl = {
                        url: ipstr,
                        operation: 'setIp'
                    }
                    resolve(executeByTerminal(sendDataurl))
                    resolve(1)
                });
    ...
    $(function(){
        ...
        //先根据缓存渲染select
        renderselect()
        if(localStorage.getItem('lastipaddress')&&(!((jumpfrom !== '0')&&(autologin == 'true')))){
                                ...
                                $.ajax({
                                    ...
                                    .then(function(res){
                                        ...
                                        if(status=='timeout'){//超时,status还有success,error等值的情况
                                            errorconsole($('.loginbox'),'登录超时请检查网络设置',0);
                                        }else if(status=='error'){
                                            errorconsole($('.loginbox'),'登录失败请检查网络设置',0);
                                        }
                                    })
