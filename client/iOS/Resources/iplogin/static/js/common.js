...
var judgeip = (function (){
               ...
               return function (ipstr){
               // 获取当前页面url
               var curPageUrl = ipstr;
 ...
 * 切换登录框下面的linkbox是否可见
 */
var isShowLinkBox = function (ipstr) {
    var isinnerip = judgeip(ipstr);
    if(isinnerip){
        if(!($('.forinnerv').hasClass('y-hide'))){
            $('.forinnerv').addClass('y-hide');
        }
    }else{
        if($('.forinnerv').hasClass('y-hide')){
            $('.forinnerv').removeClass('y-hide');
        }
    }
}
